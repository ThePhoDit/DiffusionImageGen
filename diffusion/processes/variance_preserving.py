...
class VPDiffusionProcess:
    ...
    def __init__(
        ...
        if schedule == "linear":
            ...
            c_beta_min = (
                cosine_beta_min if cosine_beta_min is not None else 1e-7
            )
            c_beta_max = (
                cosine_beta_max if cosine_beta_max is not None else 0.999
            )
            self.beta, self.alpha_bar, self.std_dev = (
            ...
            print(
                f"  Cosine schedule using beta clamp: [{c_beta_min:.1e}, {c_beta_max:.1f}]"
            )  # Add print
        else:
            raise ValueError(
    ...
    def score_fn(
        ...
        score = -(x_t - sqrt_alpha_bar_t * x_0_predicted) / torch.clamp(
            one_minus_alpha_bar_t, min=1e-7
        )
        return score
