...
class VEDiffusionProcess:
    ...
    Reverse SDE:
        dx̄t = -g(t)^2 * ∇log p_t(x̄t) dt + g(t) * dW̄t
        where ∇log p_t(x̄t) is the score function estimated by a neural network.
    ...
    def marginal_prob(
        ...
        """
        Calculates the mean and standard deviation of the perturbation kernel p(xt | x0).

        For VE SDE:
        - Mean μ(t) = x₀
        - Standard Deviation σ(t) is given by self.sigma_t(t).
    ...
    def sample_forward(
        ...
        Formula: x_t = μ(t) + σ(t) * ε, where ε ~ N(0, I).
        For VE SDE, this simplifies to: x_t = x₀ + σ(t) * ε.
        ...
        Returns:
            ...
            - noise: The standard Gaussian noise ε used to generate the sample.
        """
    ...
    def score_fn(
        ...
        """
        Calculates the score function ∇log p_t(xt) using the trained neural network.
        ...
        Returns:
            The estimated score tensor, ∇log p_t(xt).
        """
    ...
    def loss_fn(
        ...
        Loss = E_t [ λ(t) * || s_θ(x_t, t) - ∇log p_0t(x_t | x_0) ||^2 ]
        where λ(t) is a weighting function, often chosen as E[||∇log p_0t(x_t | x_0)||^2]^-1.
        For VE SDE, ∇log p_0t(x_t | x_0) = - (x_t - x_0) / sigma(t)^2 = - noise / sigma(t).
        The common loss formulation simplifies to:
        Loss = E_t [ || sigma(t) * s_θ(x_t, t) + noise ||^2 ]
        where s_θ(x_t, t) is the score predicted by the model.
        ...
        # Get noisy sample x_t and the noise ε used
    ...
    def sde_drift_reverse(
        ...
        """
        Returns the drift function f̄(x, t) for the reverse VE SDE.

        Formula: f̄(x, t) = -g(t)^2 * score_fn(x, t)
    ...
    def sde_diffusion_reverse(self) -> Callable[..., torch.Tensor]:
        """
        Returns the diffusion function ḡ(t) for the reverse VE SDE.

        Formula: ḡ(t) = g(t)
        This corresponds to the diffusion coefficient used in the sampling process.

        Returns:
            The function `self.g_t(t)`.
        """
        return self.g_t  # Same as forward diffusion coefficient
