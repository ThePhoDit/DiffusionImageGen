...
def probability_flow_ode_sampler(
    ...
    # diffusion_sq_fn = diffusion_process.diffusion_squared()
    diffusion_sq_fn = getattr(diffusion_process, 'g2_t', getattr(diffusion_process, 'diffusion_squared', None))
    if diffusion_sq_fn is None:
        raise AttributeError(f"{type(diffusion_process).__name__} object has no 'g2_t' or 'diffusion_squared' method needed for ODE sampler.")
    ...
    def _get_ode_drift(x_state, t_val_scalar):
        ...
        g2_t = diffusion_sq_fn(vec_t).view(-1, *([1] * (x_state.dim() - 1)))
        score = score_fn(score_model, x_state, vec_t, class_labels)
        ode_drift = drift_f - 0.5 * g2_t * score
        # Clamp drift magnitude for stability (especially important for Euler)
        ode_drift = torch.clamp(ode_drift, -50.0, 50.0)
        return ode_drift
    ...
    for n in time_steps_iter:
        ...
        if use_rk4:
            ...
            if torch.isnan(ode_drift).any():
                print(
                    f"!!! NaN detected in ode_drift at step {n}, t={t_current:.4f}. Stopping early. !!!"
                )
